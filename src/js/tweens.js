...
function _autoMatchJSCore(sA,sB,hist,pins,seedOverride){
  ...
  if(hyps&&hyps.length){
    ...
    for(var round=0;round<=hyps.length+1;round++){
      ...
      assign2=NemoTweenAssignment.solve(cost2);
      matches2=[];
...
function renderGhostAll(){
  ...
  for(var fi=0;fi<ld.frames.length;fi++){
    ...
    if(window.SMGroup&&ld.groups)ghostStrokes=SMGroup.applyCombinesToStrokes(ghostStrokes,ld,li,fi);
