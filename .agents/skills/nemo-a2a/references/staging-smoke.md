...
5. Require ACP to persist `processed`, perform a fresh HTTPS/NIP-98
   ...
   The local grant must bind the exact Project/channel/repository, base SHA, branch,
   worktree ID, path prefixes, capability, requester/recipient, and canonical checkout.
...
12. Exercise authorization denials for stale/expired responses, reused nonce/NIP-98 request,
    ...
    changed owner or repository-announcement event, and receiver-local capability/path scope.
