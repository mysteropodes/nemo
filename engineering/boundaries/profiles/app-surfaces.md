...
### 1c-bis. `shader-effects-library.js` — shader effect catalog, not logic (1,312 nonblank lines)

`src/js/shader-effects-library.js` had a size exception (ceiling 1312) but, unlike `i18n.js`,
its `reason` text was the generic "Legacy pre-R01 file... pending real domain/feature/adapter
layer classification" boilerplate shared with ordinary legacy files — not the "shader catalog"
language `04_MODULARITY_POLICY.md` explicitly names, and this document never analyzed the file
at all (P13/#1015 correction; previously zero mentions here despite `i18n.js`'s parallel §1c
above and Category 4's WGSL "Shader" entry below covering a *different* set of files).

Read in full to confirm the split, not assumed from the filename: lines 20-28 are two small
builder helpers, `param(key, label, min, max, step, unit, defaultValue, spatial)` and
`fx(id, name, category, params, lines)` (9 lines combined, each just assembling and returning a
plain object). Lines 30-1306 are the `effects` array itself — roughly 80 entries, each a `fx(...)`
call carrying a WGSL fragment-body string and parameter metadata — built *through* those helpers
but is itself pure data, not control flow. Lines 1308-1313 are `window.SMSHADER_EFFECTS`/
`window.SMSHADER_EFFECT_CATEGORIES` (the public exports) plus a 4-line `window.SMShaderEffectDef`
linear-search lookup. Total real control logic: ~13 lines (9 + 4) against 1312 nonblank lines —
the same "data catalog embedded in hand-written JS" shape as `i18n.js`, matching the same policy
clause ("do not split a shader catalog or translation table"). The profile's exception `reason`
now cites this directly rather than the generic legacy-pending-classification text.
...
that HTML order. `src/css/style.css` (2,797 nonblank lines) and
`src/css/tutorial.css` (159 nonblank lines) are stylesheets; `04_MODULARITY_POLICY.md` already names a
...
onto a JS-lexical checker here. Neither CSS file carries a `gitBlob`/`sha256` provenance pin the
way `src/index.html` does in `app-js.coverage.json`'s `bootstrapObservations` — re-verified at
P13/#1015 (nonblank counts recounted directly, `index.html`'s pinned hash confirmed byte-exact
against the live tree) — so their line counts here can drift silently on the next CSS edit;
flagged as a gap for a future leaf, not closed by this documentation-only pass.

| Path | Lines | Note |
|---|---:|---|
| `src/index.html` | 2455 | 132 script tags; startup order is unparsed by this tool |
| `src/css/style.css` | 2797 | exceeds policy's Stylesheet hard max (350) today |
| `src/css/tutorial.css` | 159 | within policy's Stylesheet hard max |
...
**Shader (JS catalog):** `src/js/shader-effects-library.js` — covered in §1c-bis above, same
shape as `i18n.js` below: lexically valid JS (unlike the six WGSL files just above), so it stays
in the profile with its own distinct exception reason rather than a blanket exclusion.
