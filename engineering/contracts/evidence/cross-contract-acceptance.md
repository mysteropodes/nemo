...
open, and production adoption remains gated by R03 and R05. The signed matrix
review records `origin/main` at `d4995210e8142cf6c13a3fa5dab865aca263410f`,
with none of the cited application files or job-routing lines changed.
...
its five original false-binding findings have delivered repair candidates, with
further correction and adoption still pending as recorded under separate active
dependencies below.
...
| `text` | Three glyph members share `groupId`; first glyph is the root; text-unit indices and bounds | Eleven Node checks; reviewer-run Paper.js `desP`/`serP` and group-membership round trips at `6d49921` are additional, uncommitted controls | **Candidate** corrected text-group evidence; glyphs are rectangles, one R13 pixel expectation and interactive text/selection workflows remain open |
...
- **Text grouping cleared:** three members and the first-glyph root survive actual
  ...
  These Paper.js round trips and negative controls were reviewer-run at
  `6d49921`, not committed. Negative controls detect legacy `textGroupId`, a
  detached root, and a mismatched glyph group. The [corrected builder](https://github.com/mysteropodes/nemo/blob/6d499212fdf447af439d66f5aadb905f6575110d/tests/fixtures/lib/corpus.cjs#L343-L373)
...
- **Undo benchmark finding cleared:** the [production clone loader](https://github.com/mysteropodes/nemo/blob/6d499212fdf447af439d66f5aadb905f6575110d/tests/fixtures/lib/sandbox.cjs#L116-L124)
  uses the actual `_cloneLayersForUndo` body, `_walkStrokes`, and heavy-field list.
  Reviewer-run controls at `6d49921`, not committed, verify detached heavy
  strings, restored originals, an independent nested clone, and restoration after
  stringify throws. The
...
limit remains visible at the pinned #946 commit and generation needs the
compatible build. Local source-only correction
`99158fd824b83c685dfabb6e4865becacde97145` adds explicit compatible-runtime
validation and is independently approved; generated assets and manifest are
unchanged. That review does not establish stack integration or runtime adoption.
...
**Separate active dependencies:** #944 at `dd39e52` has five runtime-confirmed
false bindings (quoted return, reassigned helper argument, anonymous callback
shadowing, block scope, and same-name helper resolution), despite 24 focused tests
passing. Repair `ebd074410f4b0b1e5c3dfe3091b637ca9c0805c6` and local follow-up
`2da73e8fbc026f464c4202957d589c94a010e372` have been delivered. Independent
review of `2da73e8` requests changes for legal multiline-initializer false
bindings; a native successor limited to the two inventory scripts is underway.
The delivered repairs do not close the inventory adoption gate.

With [#949](https://github.com/mysteropodes/nemo/pull/949) pinned at
...
first restored the expected 0.25 value, also returned by standalone #946.
Delivered draft [#959](https://github.com/mysteropodes/nemo/pull/959), pinned at
[`8faadbe5b2ea0b420e2241f280fe81088a792f03`](https://github.com/mysteropodes/nemo/commit/8faadbe5b2ea0b420e2241f280fe81088a792f03),
adds the `animation/curve.js` motion prelude, a module-naming benchmark backend,
and five sandbox regression tests wired through `nemo-fixtures.test.cjs`.
Codexalog's signed compatibility approval `64a5f3a1` covers that correction;
#959 remains stacked on unmerged #946/#944, with integration and adoption
unaccepted. The prior R08 browser result does not cover this loader and is not
revoked by it.
...
|---|---|---|---|---|
| Selection | `static-props`, `text`, and `interaction`; expected selected identities and bounds must be derived from fixture geometry and group membership. | `selectedPaths` and [document-restore refresh](https://github.com/mysteropodes/nemo/blob/1d652dd5de550bf6f5863faf5ed89c2481a36412/src/js/select-bridge.js#L3569-L3578); [text grouping](https://github.com/mysteropodes/nemo/blob/1d652dd5de550bf6f5863faf5ed89c2481a36412/src/js/vector-text-bridge.js#L350-L358). | Reorder selection identity has an integrated Node check. Reviewer-run controls at `6d49921`, not committed tests, passed the real group-membership consumer after Paper.js serialization round trips. The committed evaluator builds plain objects shaped like `desP` output. Interactive pointer selection is still unrun. | Browser/Paper harness: load, click/marquee/group-select, edit, undo/redo, reorder/duplicate, and assert selected stable IDs after each rebuild. |
| Animation | `keyed-props`, `components`, `expression-props`, `held-frames`, and `mesh`. Expected values/tables are recorded independently. | [`motion.js` track writers/readers](https://github.com/mysteropodes/nemo/blob/1d652dd5de550bf6f5863faf5ed89c2481a36412/src/js/motion.js#L2747-L2776), `tweens.js`, symbol frame resolution, and per-element/mesh motion. | Integrated easing tests and candidate production VM motion/expression/component checks cover reads. Draft [#959](https://github.com/mysteropodes/nemo/pull/959) at `8faadbe` delivers the #949 loader correction with compatibility approval; stack integration/adoption, key writes, browser sandbox/UI, history, persistence, and rendered positions remain open. | Integration plus browser harness: key/edit/undo/redo/save/load, evaluate named frames, then compare rendered transforms and exact frame timing. |
...
#944 correction/adoption gate after delivered repairs `ebd07441` and `2da73e8`,
with the latter under REQUEST CHANGES, plus stack integration/adoption of the
delivered #949 loader correction in draft #959 `8faadbe`. The independently
approved local PNG-runtime correction `99158fd` likewise awaits integration;
none of these source dispositions clears the runtime gates above.
