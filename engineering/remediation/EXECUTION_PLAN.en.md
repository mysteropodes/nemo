...
The source audit used `66ece0641708122eb8447e85ad8dd7e3402aaf6c`, not the older local main checkout. Reuse the honest F0 baseline, merged curve extraction, project parser, opacity application service, existing consumer tests and Rust MCP transport. P04's descriptor schema, D02's lifecycle contract and P05's deterministic feature registry are merged; the existing `nemo-mcp/src/registry.rs` still locates application instances. P06/#1008 and P07/#1009 remain open: their first candidates received Ilya/O changes requested and corrective work remains under Ilya/O review. D01/#1044 remains owned by Ilya/O, and T05/#1054 remains reserved to Pollen. The 147-module JS profile still classifies 140 modules as legacy. These are source observations, not newly executed test results.
