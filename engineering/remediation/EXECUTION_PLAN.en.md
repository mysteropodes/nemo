...
- [x] **Board first:** create [R23 / #1327](https://github.com/mysteropodes/nemo/issues/1327) and the bounded N00–N05 queue in canonical [Project #2](https://github.com/users/mysteropodes/projects/2), with native sub-issue and blocked-by relations, complete fields and an updated project README. The current 20 September readback confirms R23 and N05 `In progress`, with N00–N04 `Already there` / `Done` / `Accepted`; the admitted N06–N21 states are recorded below.
...
N00–N04 have terminal acceptance receipts: N00 merge `2bd3da7f7e7811b51150bcf95dc669182d2c8f25`, N01 metadata/Project reconciliation, N02 merge `c489e5fbb1822f6a816db3a4d8e901d67382b7a2`, N03 merge `b37b9cb6a4eb391a248a8dba98cf3f676afbbfe9`, and N04 merge `3084dbf9aeb7010b4ecd24eafee57c106f57c32f`. Their issue receipts retain exact candidates, checks and limitations. **N05 remains open pending acceptance of this queue and synchronized plan.** N03 is a characterized headless proof that rejects changed opacity; N04 is a macOS viewport proof. Neither is production migration acceptance.

The live issue ledger after creating N06–N21 contains **171 leaves = 142 accepted + 29 open (83.04%)**, before N05 closure. After N05 is accepted, and only then, it becomes **143/171 (83.63%)**. These are accepted issue outcomes, including preparation and validation; the first native slice is separately **0/16 runtime accepted**. P03 census packets are not this denominator, and neither packet counts nor this first slice establish a complete whole-engine migration denominator.

All sixteen new leaves belong to **Ilya (`ivg-design`)**, with **Ilya/O as Validation owner**. O uses `gpt-6-astra/high`, D1 `gpt-5.6-sol/high`, D2 `gpt-5.6-terra/high`; record actual settings at claim. The owning team assigns an independent reviewer before writing. New local agent names include model/effort. N06 has an active shared-file impediment: Project #2 Status `Needs work`, Board Status `Needs work`, Remediation `Blocked`, Validation `Planned` until both N05 acceptance and the terminal #1316/PR #1323 handoff. N07–N21 remain Remediation `Inbox`, Status/Board Status `Planned later`, Validation `Planned`; advance only from accepted predecessor evidence. At each claim record the actual predecessor merge SHAs, not prospective hashes. Numeric order is not readiness.
...
| [N19 / #1351](https://github.com/mysteropodes/nemo/issues/1351) | Enforce native boundaries and local validation | R18.3 / O | N13, N15, N16, N17, N18 |
| [N20 / #1352](https://github.com/mysteropodes/nemo/issues/1352) | Atomically cut over the supported opacity subset | R18.1 / O | N17, N18, N19 |
| [N21 / #1353](https://github.com/mysteropodes/nemo/issues/1353) | Accept installed native and browser boundaries | R18.4 / O | N20 |

**Whole-file claims pass through explicit serialized handoffs.** Each linked issue contains its exact new/existing path list, consumer matrix, checks and cutover condition. N06 requires both accepted N05 and the terminal ownership handoff of #1316/PR #1323. Rust source registration then follows **N06→N07→N08→N09→N10→N11→N12→N13→N14→N15→N16**; JS adapter registration follows **N13→N15→N17→N18**, with the intervening dependencies in the table. A predecessor must release its full-file claim before the next writer starts; different module files do not permit concurrent edits to shared profiles.
...
**N19 owns only** `engineering/boundaries/profiles/native-engine.edges.json`, `tests/native-engine-enforcement.test.cjs`, `scripts/nemo/ci.cjs`, `scripts/nemo/lib/jobs.cjs` and `package.json` for normal job registration and negative controls. It does not own the shared source-registration profiles or a direct #1316 handoff; that prerequisite is inherited through N06. N20 retains the existing project/document, tween, opacity bootstrap/domain/capability, motion/timeline, app, render/export and `src/index.html` cutover files listed in #1352. N21 owns only its new desktop and browser acceptance tests. Any additional feature implementation discovered during cutover must be split and staged before N20 is Ready.
...
N07 covers save/load and identity; N08–N09 cover commands, retry, transactions and history; N10 covers explicit time/context evaluation; N11–N14 cover scheduling, GPU/resource lifetime, input/presentation and pinned export; N15–N18 cover the same UI/MCP dispatch and staged consumer ports. N19 supplies meaningful failing unregistered-source, forbidden-dependency and missing-job controls. N20/N21 cover the combined save/load, undo/redo, selection, animation, render, export, native bridge, browser and installed-desktop matrix at one identified candidate. Stale revisions, changed-body retries, replacement, unsupported content, stale generations and cancellation/cleanup failures remain explicit negative controls.

**Activation and validation.** Before N20, production documents keep their existing owner; staged modules and transports are not a second writable document. N20 admits only the characterized supported subset atomically and makes its former JS writers/evaluator unreachable in the same outcome. Unsupported documents fail admission before transfer and retain one explicitly legacy owner; no silent fallback or editable mirror is allowed. Other document families remain open migration work. Each leaf runs its named locked Cargo/Node tests, formatting and `npm run check`; N06 freezes the exact feature/target commands. N16 adds combined native/MCP host checks; N19 registers normal enforcement; N21 records the identified installed executable and real-browser result separately. Unit tests, a fixture host, compilation, screenshots or merge alone do not establish installed acceptance. No hosted build is authorized.
