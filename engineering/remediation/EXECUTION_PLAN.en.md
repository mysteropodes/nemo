...
- [x] **Board first:** create [R23 / #1327](https://github.com/mysteropodes/nemo/issues/1327) and the bounded N00–N05 queue in canonical [Project #2](https://github.com/users/mysteropodes/projects/2), with native sub-issue and blocked-by relations, complete fields and an updated project README. The current 20 September readback confirms R23 `In progress`, with N00–N05 `Already there` / `Done` / `Accepted`; the admitted N06–N21 states are recorded below.
- [x] **[N00 / #1329](https://github.com/mysteropodes/nemo/issues/1329):** publish this synchronized English/French amendment and the active repository-entry rules. No engine source work starts before N00 is reviewed, merged and accepted.
- [x] **[N01 / #1328](https://github.com/mysteropodes/nemo/issues/1328):** after N00, rebaseline the live remediation queue against current main and map every retained, replaced or newly blocking leaf without erasing accepted receipts or current ownership.
- [x] **[N02 / #1331](https://github.com/mysteropodes/nemo/issues/1331):** after N00, freeze the native engine, revision, command/query/job, transition and adapter contracts, including two concrete compatibility/failure examples and explicit authority boundaries.
...
The initial board contained **59 leaves: 33 owned by Ilya and 26 by Cyrill**. These historical counts describe the starting queue; the current admitted ledger is below.

### N — admitted first native opacity slice (N05, 20 September 2026)

N00–N05 have terminal acceptance receipts: N00 merge `2bd3da7f7e7811b51150bcf95dc669182d2c8f25`, N01 metadata/Project reconciliation, N02 merge `c489e5fbb1822f6a816db3a4d8e901d67382b7a2`, N03 merge `b37b9cb6a4eb391a248a8dba98cf3f676afbbfe9`, N04 merge `3084dbf9aeb7010b4ecd24eafee57c106f57c32f`, and N05 merge `cf73104ad896ea2b7c90d5115cc77416d6c6a13b`. Their issue receipts retain exact candidates, checks and limitations. N03 is a characterized headless proof that rejects changed opacity; N04 is a macOS viewport proof. Neither is production migration acceptance.

At the N05 acceptance snapshot, the live issue ledger contained **171 leaves = 143 accepted + 28 open (83.63%)**. These are accepted issue outcomes, including preparation and validation; the first native slice was separately **0/16 runtime accepted**. The N16 review later admitted N18A/#1367 as a seventeenth runtime leaf. N20 preflight then proved that authoritative serialization/evaluation reads and a terminal native release/re-entry contract were missing, so N19A/#1373 and N19B/#1374 were admitted as the eighteenth and nineteenth runtime leaves before N20. The subsequent callback-reachability review admitted N19C/#1377, N19D/#1378 and N19E/#1379 as runtime leaves twenty through twenty-two: N19C installs the dormant pre-mutation legacy-edit barrier, N19D serially extends that barrier to selection and fallback mutations after N19C, and N19E independently makes external project-replacement callers await their terminal result after N19B. Live issue and Project #2 state supersede these historical snapshot counts. P03 census packets are not this denominator, and neither packet counts nor this first slice establish a complete whole-engine migration denominator. The N20 exact-candidate review admitted N19F/#1384 as runtime leaf twenty-three to require acknowledgment before bundled MCP wire success.

All twenty-three admitted native leaves belong to **Ilya (`ivg-design`)**, with **Ilya/O as Validation owner**. O uses `gpt-6-astra/high`, D1 `gpt-5.6-sol/high`, D2 `gpt-5.6-terra/high`; record actual settings at claim. The owning team assigns an independent reviewer before writing. New local agent names include model/effort. At the N05 snapshot, N06 retained one active shared-file impediment and N07–N21 remained planned; N18A was admitted later from the N16 review, N19A/N19B were admitted from N20 preflight, and N19C/N19D/N19E were admitted from callback-reachability review. Advance each leaf only from accepted predecessor evidence and use its live issue and Project #2 fields for current state. At each claim record the actual predecessor merge SHAs, not prospective hashes. Numeric order is not readiness.

| Leaf | Observable outcome | Family / lane | Actual predecessors |
|---|---|---|---|
| [N06 / #1338](https://github.com/mysteropodes/nemo/issues/1338) | Scaffold crate and module seams | R18.1 / O | N05, #1316 / PR #1323 |
| [N07 / #1339](https://github.com/mysteropodes/nemo/issues/1339) | Admit and round-trip opacity documents | R18.1 / D1 | N06 |
| [N08 / #1340](https://github.com/mysteropodes/nemo/issues/1340) | Own revisions and opacity commands | R18.1 / D1 | N07 |
| [N09 / #1341](https://github.com/mysteropodes/nemo/issues/1341) | Own transactions and history | R18.1 / D1 | N08 |
| [N10 / #1342](https://github.com/mysteropodes/nemo/issues/1342) | Evaluate immutable frame/context inputs | R18.2 / D2 | N08, N09 |
| [N11 / #1343](https://github.com/mysteropodes/nemo/issues/1343) | Schedule frames and release resource leases | R18.3 / D2 | N10 |
| [N12 / #1344](https://github.com/mysteropodes/nemo/issues/1344) | Share one native preview/export compositor | R18.3 / D2 | N11 |
| [N13 / #1345](https://github.com/mysteropodes/nemo/issues/1345) | Provide viewport host and input adapter | R18.4 / D2 | N12 |
| [N14 / #1346](https://github.com/mysteropodes/nemo/issues/1346) | Export pinned-revision PNG frames | R18.5 / D1 | N09, N12, N13 |
| [N15 / #1347](https://github.com/mysteropodes/nemo/issues/1347) | Expose the common v2 dispatcher/protocol | R18.6 / D1 | N09, N14 |
| [N16 / #1348](https://github.com/mysteropodes/nemo/issues/1348) | Connect Tauri/MCP host transports without cutover | R18.6 / O | N13, N15 |
| [N17 / #1349](https://github.com/mysteropodes/nemo/issues/1349) | Stage editor and selection intent adapters | R18.4 / D1 | N09, N10, N15, N16 |
| [N18 / #1350](https://github.com/mysteropodes/nemo/issues/1350) | Stage preview and export consumer adapters | R18.3 / D2 | N12, N13, N14, N15, N16, N17 |
...
| [N20 / #1352](https://github.com/mysteropodes/nemo/issues/1352) | Atomically cut over the supported opacity subset | R18.1 / O | N17, N18, N18A, N19, N19A, N19B, N19C, N19D, N19E, N19F |
...
**Whole-file claims pass through explicit serialized handoffs.** Each linked issue contains its exact new/existing path list, consumer matrix, checks and cutover condition. N06 requires both accepted N05 and the terminal ownership handoff of #1316/PR #1323. Rust source registration follows **N06→N07→N08→N09→N10→N11→N12→N13→N14→N15→N16**, then the serialized **N18A→N19→N19A→N19B→N19C→N19D→N20** handoff after accepted N17/N18; N19E proceeds in parallel after N19B and joins N20 only after its own acceptance. N19F proceeds after accepted N19B and the shared-file handoff, and is a required accepted predecessor of N20. JS/application registration follows **N13→N15→N17→N18→N18A→N19A→N19C→N19D**, with the intervening dependencies in the table. Shared app-JS provenance and generated-inventory writes serialize **N19E→N19C**: N19E refreshes them for its accepted caller sources first, then releases them for N19C's guard source after handoff. A predecessor must release its full-file claim before the next writer starts; different module files do not permit concurrent edits to shared profiles.

N06 alone owns `native-engine/Cargo.toml`, `Cargo.lock`, `src/lib.rs` and `README.md`: it freezes dependencies, disabled feature-gated module exports and required-feature test targets, without placeholder runtime behavior. N07–N15 own their distinct implementation modules and focused tests without rewriting N06's scaffold. In addition, **each Rust-source candidate N06–N15** reserves and updates `engineering/boundaries/profiles/rust.profile.json`, `engineering/boundaries/profiles/rust.baseline.json`, `engineering/boundaries/profiles/rust.coverage.json` and `tests/nemo-boundaries-rust.test.cjs` as fresh source discovery requires, after its predecessor's handoff. Registration accompanies the source candidate; it is not postponed to N19.

**N13, N15, N17 and N18** likewise reserve `engineering/boundaries/profiles/app-js.profile.json`, `engineering/boundaries/profiles/app-js.baseline.json`, `engineering/boundaries/profiles/app-js.coverage.json`, `engineering/boundaries/profiles/app-js.edges.json` and `scripts/nemo/boundaries-application.test.cjs` for each fresh adapter registration, with serialized full-file handoff. N16 owns the existing Tauri/MCP manifest, lock, host, wire/schema and transport-test wiring plus `src/js/adapters/application-mcp.js`, and updates `engineering/boundaries/profiles/mcp-rust.profile.json` and `engineering/boundaries/profiles/nemo-desktop.edges.json` for those hosts. It connects transports without admitting native document ownership. N17 adds the `native-opacity-editor.js` / `native-opacity-selection.js` adapters and their test; N18 adds `native-opacity-preview.js` / `native-opacity-export.js` and their test.

**N18A owns the dormant production-selectable native application bootstrap and concrete desktop host ports discovered by the N16 review.** It may add one narrow native snapshot accessor, retain one optional native application and one main-window viewport, and make UI plus authenticated bundled MCP reach that same application. Its viewport and export compositor share one exact GPU context. N18A updates the Rust source profile/coverage and desktop edges with per-module native-engine allowlists while leaving the frozen Rust baseline unchanged. It must not activate the bootstrap, admit a production document, create a second writable authority or claim installed proof; N20 owns invocation/cutover and N21 owns installed acceptance.

**N19 owns only** `engineering/boundaries/profiles/native-engine.edges.json`, `engineering/boundaries/profiles/rust.profile.json`, `tests/native-engine-enforcement.test.cjs`, `scripts/nemo/ci.cjs`, `scripts/nemo/ci.test.cjs`, `scripts/nemo/lib/jobs.cjs`, `engineering/inventory/baseline-manifest.json`, the three generated inventory artifacts `engineering/inventory/surfaces.json`, `engineering/inventory/surfaces.csv` and `engineering/inventory/SURFACES.md`, `package.json` and the synchronized English/French N19 plan paragraphs for normal job registration and negative controls after N18A releases its shared files. Its narrow profile exception repartitions the already registered `native-engine/src` paths so the adopted Rust analyzer can enforce internal direction. The generated inventory artifacts bring the already accepted current source surfaces back into sync after N19's normal-validation run exposed stale outputs; N19 changes no inventory source itself. N19 may not change the registered path set, size budgets, frozen baseline, coverage policy/exclusions, predecessor gate tests or production sources. N19 does not receive a direct #1316 handoff; that prerequisite is inherited through N06.
...
N20 retains the existing project/document, tween, opacity bootstrap/domain/capability, motion/timeline, app, render/export and `src/index.html` cutover files listed in #1352, plus the feature declaration whose availability it activates. N20 installs the accepted N19C/N19D release controller, attaches `SMEngineBridge.nativeEditGuard` before any native-authority activation, and wires only the accepted N19A read ports, N19B release/re-entry port, N19C/N19D guard seam and N19E awaited callers; the cutover tests freeze that ordering. It must not add missing Rust/host feature behavior, strip unsupported content into a falsely supported projection or weaken admission. N21 owns only its new desktop and browser acceptance tests. Any additional feature implementation discovered during cutover must be split and staged before N20 is Ready.
...
- [ ] **[P03 / #1005](https://github.com/mysteropodes/nemo/issues/1005) — Validate the combined source census and admit the complete extraction queue**
  ...
  Admission is split before implementation: [P03A / #1116](https://github.com/mysteropodes/nemo/issues/1116) freezes the source index and rejects false completeness claims; [P03B / #1170](https://github.com/mysteropodes/nemo/issues/1170) refreezes it at current `main` with the C19/C20 supplements pinned, records the recomputed per-file range coverage (21 422 code lines in 171 spans still belong to no packet — `app.js`, the `timeline.js` tail and `motion.js` first) and admits reviewed dispositions only with their evidence. [Index contract](../inventory/REMEDIATION_SCOPE.md). P03 remains open for P03C: reconcile the five open notes and range drift, map the unmapped monoliths as bounded census slices, then split pending packets into ≤90-minute leaves or disposition them; pending packet references are not Ready tasks.
...
- [ ] **[P33 / #1035](https://github.com/mysteropodes/nemo/issues/1035) — Accept the completed fixed census against integrated evidence**
  ...
  Owner **Ilya/O** · skill `orchestration` · `gpt-6-astra` / **high**. Predecessors: [P03 / #1005](https://github.com/mysteropodes/nemo/issues/1005), [P08 / #1010](https://github.com/mysteropodes/nemo/issues/1010), [P32 / #1034](https://github.com/mysteropodes/nemo/issues/1034). All N06–N21 leaves above are also required accepted outcomes; no parent percentage substitutes for their receipts.
