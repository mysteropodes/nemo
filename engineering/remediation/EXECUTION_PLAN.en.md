...
| Slot | Ilya | Cyrill |
|---|---|---|
| **O — orchestrator** | `gpt-6-astra`, **high**. Own shared contracts, source/coverage policy, integration order, combined validation and the canonical board. | `opus`, **high**. Review at milestones; own team claims, existing Fizz/Honey handoffs, Claude/native acceptance and quota decisions. |
...
- [ ] Delegates update only their own leaf. Orchestrators handle shared integration, reassignment, parent summaries, dependency release and final closure. A human ownership transfer changes issue assignee, plan allocation and current handoff together.
...
- [ ] No direct main push, protection bypass, automatic hosted build, release or deployment. A specific hosted run still needs an explicit human request. Ordinary commits, pushes and PRs are not that request.
- [ ] Main currently requires an approving review and approval of the last push. The local orchestrator performs technical review; when the author shares its GitHub account, the other human supplies the required independent GitHub approval. Publish a normal PR with local evidence; wait for that review rather than bypassing protection. A branch published for review is not merged acceptance.
...
The separately authorized Buzz update adds a persistent workspace instruction editor using the existing signed Project settings, with built-in fallback/reset and visible revision. Only the Project's signing owner edits the shared value; other members use their own Team Instructions for differences. Do not relabel a per-device value as shared or grant broader access to make the editor work. Release, installer and runtime-uptake evidence must be recorded before claiming this update installed on either Mac. Broader Buzz transport changes remain outside Nemo remediation.
...
- [ ] In your team instructions, set your human name, report language, model/effort defaults and quota policy from the table. Save; apply at a safe checkpoint and verify the effective revision on the next agent turn. Do not restart a busy task simply because the settings changed.
...
**Cyrill's installer packet:** the tested release version, checksum, Google Drive download link and verified effective-instruction behavior will be inserted here when the Buzz build and Ilya's link are available. This is an explicit pending delivery item, not a prerequisite to independent Nemo inventory/test work.
