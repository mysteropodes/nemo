...
- [ ] Dans les instructions d’équipe, indiquer votre nom, la langue des rapports, les valeurs par défaut du modèle/de l’effort et la politique de quota d’après le tableau. Sauvegarder ; appliquer à un point de reprise sûr et vérifier la révision effective au prochain tour de l’agent. Ne pas redémarrer une tâche occupée simplement parce que les paramètres ont changé.
...
**Dossier d’installation de Cyrill :** la version testée, la somme de contrôle, le lien de téléchargement Google Drive et le comportement vérifié de prise en compte des instructions seront insérés ici lorsque le build Buzz et le lien d’Ilya seront disponibles. C’est une livraison explicitement en attente, pas un prérequis aux travaux indépendants d’inventaire/tests Nemo.
