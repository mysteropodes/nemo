...
all **699 Git-tracked paths** at `1bfcbad1eea9ab810ee87cfd6bc18306b2533ee8`. Every path has one
...
**0 removed**, **20 modified** paths). The checker recomputes the last delta from the two Git
...
All **38 census files** present at the frozen commit are pinned by blob and keep their own
`source_sha` — the 14 original partitions plus C19a–C19q (timeline) and C20a–C20f (Motion).
An unpinned census file in the tree fails integrity. The pins yield **464 packets**: 329 from
the original partitions (C08 keeps only its documentation map) and 135 from the supplements.
...
At `1bfcbad`, **61 files** carry range declarations, **34** have gaps, and **21 422 code lines in
...
|---|---:|---:|---:|
| `src/js/timeline.js` | 11 907 | 5 286 | 11 (all from 5710 onward except 2002-2011, 2027-2048, 2247) |
...
C20), `tweens.js` (158, C01 versus C02), `timeline.js` (87, C19 versus C02/C06).

## Dispositions with evidence

Every reviewed claim carries its evidence and is validated by shape:

- **Packets** — `pending` (464), `covered` (leaf + issue + merged PR + evidence), `admitted`
  (issue) or `deferred` (reason). No delivered leaf matched an entire packet: P20, P21, P22, P23,
  P26, P28, A01, P17 and P18 each extracted part of a larger packet, so all 464 stay pending
...
`timeline.js` tail from 5710 and the `motion.js` spans — as further bounded census slices;
