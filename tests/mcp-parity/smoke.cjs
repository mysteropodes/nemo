...
// P16 runs this same chain against two different sets of bytes: the freshly
// built binary and the one inside an installed distribution. The comparison is
// the point, so the binary under test is selectable instead of hardcoded.
