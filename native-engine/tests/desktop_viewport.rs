...
use crate::render_scene::{prepare, GeometryPaintInput, LayerGeometry, OpaqueSrgbPaint, ScheduledFrameIdentity};
use crate::scheduler::{EvaluationKey, FrameFailure, FrameFailureKind, FrameScheduler, OutputSpec, PublicationDisposition};
...
#[derive(Clone, Copy)]
enum Step { Presented, Lost, Outdated, Timeout, Occluded, Validation, DeviceLost }
struct FakePort { steps: VecDeque<Step>, events: RefCell<Vec<&'static str>>, recreates: usize, reconfigures: usize, disposals: usize, settles: usize, fail_recreate: bool, fail_reconfigure: bool, presentations: Vec<(PhysicalExtent, PhysicalExtent, SurfaceScalePolicy)> }
impl Default for FakePort { fn default() -> Self { Self { steps: VecDeque::new(), events: RefCell::new(Vec::new()), recreates: 0, reconfigures: 0, disposals: 0, settles: 0, fail_recreate: false, fail_reconfigure: false, presentations: Vec::new() } } }
struct FakeFrame<'a> { port: Option<&'a mut FakePort>, outcome: SettledSurfaceFrame, consumed: bool }
impl AcquiredSurfaceFrame for FakeFrame<'_> { fn settle(mut self) -> SettledSurfaceFrame { let outcome = self.outcome; self.port.as_deref_mut().unwrap().events.borrow_mut().push("settle"); self.port.as_deref_mut().unwrap().settles += 1; self.consumed = true; outcome } }
impl Drop for FakeFrame<'_> { fn drop(&mut self) { if let Some(port) = self.port.take() { port.events.borrow_mut().push(if self.consumed { "frame-dropped" } else { "frame-leaked" }); } } }
impl FakePort {
    fn queue(&mut self, steps: &[Step]) { self.steps.extend(steps.iter().copied()); }
    fn attempt(&mut self) -> SurfaceAttempt<FakeFrame<'_>> {
        ...
        match self.steps.pop_front().unwrap_or(Step::Presented) {
            Step::Presented => SurfaceAttempt::Acquired(FakeFrame { port: Some(self), outcome: SettledSurfaceFrame::Presented, consumed: false }),
            Step::Validation => SurfaceAttempt::Acquired(FakeFrame { port: Some(self), outcome: SettledSurfaceFrame::ValidationFailure, consumed: false }),
            Step::DeviceLost => SurfaceAttempt::Acquired(FakeFrame { port: Some(self), outcome: SettledSurfaceFrame::DeviceLost, consumed: false }),
            Step::Lost => SurfaceAttempt::Lost, Step::Outdated => SurfaceAttempt::Outdated, Step::Timeout => SurfaceAttempt::Timeout, Step::Occluded => SurfaceAttempt::Occluded,
        }
    }
}
impl SurfacePort for FakePort {
    type Frame<'port> = FakeFrame<'port>;
    fn acquire<'port>(&'port mut self, source: SurfacePresentation<'_>) -> SurfaceAttempt<Self::Frame<'port>> { self.presentations.push((source.source_extent, source.surface_extent, source.scale_policy)); self.attempt() }
    fn recreate(&mut self, _: PhysicalExtent) -> Result<(), ()> { self.events.borrow_mut().push("recreate"); self.recreates += 1; (!self.fail_recreate).then_some(()).ok_or(()) }
    fn reconfigure(&mut self, _: PhysicalExtent) -> Result<(), ()> { self.events.borrow_mut().push("reconfigure"); self.reconfigures += 1; (!self.fail_reconfigure).then_some(()).ok_or(()) }
    fn dispose(&mut self) { self.events.borrow_mut().push("dispose"); self.disposals += 1; }
    fn simulated_acquire<'port>(&'port mut self, _: &ScheduledFrameIdentity) -> SurfaceAttempt<Self::Frame<'port>> { self.attempt() }
}

fn mapping(physical: PhysicalExtent, composition: PhysicalExtent, dpr: f64) -> ViewportMapping { ViewportMapping::new(CssBounds { x: 100.0, y: 50.0, width: 640.0, height: 360.0 }, physical, composition, dpr).unwrap() }
fn standard() -> ViewportMapping { mapping(PhysicalExtent { width: 1280, height: 720 }, PhysicalExtent { width: 320, height: 180 }, 1.25) }
fn app() -> NativeOpacityApplication { NativeOpacityApplication::new("viewport", decode_project(PROJECT).unwrap()).unwrap() }
fn identity(app: &NativeOpacityApplication, scheduler: &mut FrameScheduler, frame: u32) -> ScheduledFrameIdentity {
    let snapshot = app.acquire_snapshot(0).unwrap();
    let key = EvaluationKey::new(snapshot.id(), "scene-root", frame, "final", OutputSpec::new("frame", "rgba8", 320, 180, "srgb", "straight").unwrap(), Vec::<(String, String)>::new()).unwrap();
    ScheduledFrameIdentity::from_scheduled(&scheduler.schedule(snapshot, key).unwrap())
}
fn queue(host: &mut DesktopViewportHost<FakePort>, steps: &[Step]) { host.test_port_mut().queue(steps); }
fn fail(scheduler: &mut FrameScheduler, id: &ScheduledFrameIdentity) { assert_eq!(scheduler.fail(id.work_id(), FrameFailure::new(FrameFailureKind::Worker, "viewport failure")).unwrap().publication(), &PublicationDisposition::NotPublished); }
fn assert_leases(scheduler: &FrameScheduler, exact: u64) { let counters = scheduler.lease_counters(); assert_eq!((counters.acquired(), counters.released(), counters.live()), (exact, exact, 0)); }
...
fn retry_configuration_and_resize_failures_latch_without_port_reentry() {
    ...
    let recreate = identity(&app, &mut scheduler, 20); let mut host = DesktopViewportHost::new(FakePort::default(), standard()); host.register(recreate.clone()).unwrap(); host.test_port_mut().fail_recreate = true; queue(&mut host, &[Step::Lost]); assert_eq!(host.simulate_port_completion(recreate.clone()).unwrap().status(), &ViewportStatus::Failed(FatalAction::RecreateFailed)); fail(&mut scheduler, &recreate);
    let reconfigure = identity(&app, &mut scheduler, 21); let mut host = DesktopViewportHost::new(FakePort::default(), standard()); host.register(reconfigure.clone()).unwrap(); host.test_port_mut().fail_reconfigure = true; queue(&mut host, &[Step::Outdated]); assert_eq!(host.simulate_port_completion(reconfigure.clone()).unwrap().status(), &ViewportStatus::Failed(FatalAction::ReconfigureFailed)); fail(&mut scheduler, &reconfigure);
    let mut resize = DesktopViewportHost::new(FakePort::default(), standard()); let same = mapping(PhysicalExtent { width: 1280, height: 720 }, PhysicalExtent { width: 640, height: 360 }, 2.0); resize.resize(same).unwrap(); assert_eq!(resize.test_port().reconfigures, 0); let old = resize.mapping(); resize.test_port_mut().fail_reconfigure = true; let changed = mapping(PhysicalExtent { width: 640, height: 360 }, PhysicalExtent { width: 320, height: 180 }, 2.0); assert_eq!(resize.resize(changed), Err(ViewportError::Fatal(FatalAction::ReconfigureFailed))); assert_eq!(resize.mapping(), old); assert_leases(&scheduler, 4);
}
